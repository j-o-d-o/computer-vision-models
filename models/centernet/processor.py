...
class ProcessImages(IPreProcessor):
    ...
    def fillHeatmap(self, ground_truth, cls_idx, center_x, center_y, obj_width, obj_height, mask_width, mask_height):
        ...
        for x in range(min_x, max_x):
            for y in range(min_y, max_y):
                var_width = math.pow(((self.params.VARIANCE_ALPHA * obj_width) / (6 * self.params.R)), 2)
                var_height = math.pow(((self.params.VARIANCE_ALPHA * obj_height) / (6 * self.params.R)), 2)
    ...
    def calcImgData(self, obj, roi, mask_width, mask_height):

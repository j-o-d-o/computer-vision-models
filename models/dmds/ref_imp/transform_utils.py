...
def matrix_from_angles(rot):
    ...
    This function mimics _euler2mat from struct2depth/project.py, for backward compatibility, but wraps tensorflow_graphics instead of reimplementing it.
...
def angles_from_matrix(matrix):
    ...
    Args:
      matrix: A tf.tensor of shape [..., 3, 3], where the last two dimensions are the rotation matrix.

    Returns:
      A tf.Tensor of shape [..., 3], where the last dimension is the rotation angles, along x, y, and z.

    This function mimics _euler2mat from struct2depth/project.py, for backward compatibility, but wraps tensorflow_graphics instead of reimplementing it. The negation and transposition
     are needed to bridge the differences between the two.
    """
    rank = tf.rank(matrix)
    # Swap the two last dimensions
...
def unstacked_matrix_from_angles(rx, ry, rz, name=None):
    ...
    Returns:
      A 3-tuple of 3-tuple of tf.Tensors of the same shape as x, representing the respective rotation matrix. The small 3x3 dimensions are unstacked into a tuple to avoid tensors with small
       dimensions, which bloat the TPU HBM memory. Unstacking is one of the recommended methods for resolving the problem.
    """
    angles = [-rx, -ry, -rz]
...
def invert_rot_and_trans(rot, trans):
    ...
    Returns:
      a tuple (inv_rot, inv_trans), representing rotation angles and translation vectors, such that applting rot, transm inv_rot, inv_trans, in succession results in identity.
    """
...
def combine(rot_mat1, trans_vec1, rot_mat2, trans_vec2):
    ...
    """
    # Building a 4D transform matrix from each rotation and translation, and multiplying the two, we'd get:
    #
    # (  R2   t2) . (  R1   t1)  = (R2R1    R2t1 + t2)
    # (0 0 0  1 )   (0 0 0  1 )    (0 0 0       1    )
    #
    # Where each R is a 3x3 matrix, each t is a 3-long column vector, and 0 0 0 is a row vector of 3 zeros. We see that the total rotation is R2*R1 and the total translation is R2*t1 + t2.
